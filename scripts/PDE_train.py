...
from tictoc import bench_dict, start_bench
...
def main(args):
    ...
    generator_val = tree_dataset_cloud(
        val_path, cfg.TRAIN.N_POINTS, normal_filter=True, distances=1, return_trunk = cfg.DATA_CREATION.STICK
    )
    ...
    try:
        for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
            ...
            torch.cuda.empty_cache()
            write_losses(
                scheduler, writer, epoch, temp_loss, loss_dict, val_loss, val_loss_dict
            )

            if (
                (temp_loss < init_loss)
                or (epoch % 10 == 0)
                or (val_loss < init_val_loss)
            ):
                save_checkpoint(
                    DeepPointwiseDirections,
                    scaler,
...
def train_one_epoch(
    model, epoch, generator, opt, scaler, scheduler, args, use_amp, cfg
):
    ...
    for i, (batch_train_data, batch_direction_label_data, _) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss_dict["distance_loss"] += (
                Loss_torch.distance_loss(y, batch_direction_label_data, classif=cfg.MODEL.CLASS_SIGMOID).detach() * 0.2
            )
            if cfg.TRAIN.DISTANCE:
                total_loss += (
                    Loss_torch.distance_loss(y, batch_direction_label_data, classif=cfg.MODEL.CLASS_SIGMOID) * 0.2
                )
                if cfg.MODEL.CLASS_SIGMOID:
                    pred_labels = y[:,3] > 0.5
                    gt_labels = batch_direction_label_data[:,:,3]
                    loss_dict["accuracy"] += torch.sum(pred_labels==gt_labels)/torch.multiply(*gt_labels.shape)
...
def load_checkpoint(cfg, DeepPointwiseDirections, scaler, optomizer, scheduler):
    ...
    DeepPointwiseDirections.load_state_dict(
        checkpoint["model_state_dict"]
    ) if checkpoint.get("model_state_dict", False) else None
    start_epoch = checkpoint["epoch"] if checkpoint.get("epoch", False) else None
    optomizer.load_state_dict(checkpoint["optimizer_state_dict"]) if checkpoint.get(
        "optimizer_state_dict", False
    ) else None
    scaler.load_state_dict(checkpoint["scaler"]) if checkpoint.get(
        "scaler", False
    ) else None
    scheduler.load_state_dict(checkpoint["scheduler"]) if checkpoint.get(
        "scheduler", False
    ) else None
    init_loss = checkpoint["loss"] if checkpoint.get("loss", False) else None
    init_val_loss = (
        checkpoint["val_loss"] if checkpoint.get("val_loss", False) else None
    )
