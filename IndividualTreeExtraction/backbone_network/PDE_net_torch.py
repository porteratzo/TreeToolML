...
class relation_reasoning_layers(nn.Module):
    def __init__(self,name, inputs, is_training, bn_decay, nodes_list, weight_decay, is_dist):
...
def get_relation_features(point_features, nn_idx, k):
    ...
    for i in range(num_vertex_pairs):
        ...
        temp_concat = torch.cat([point_cloud_neighbors[:, :, vertex_pairs_list[i][0], :],
                                point_cloud_neighbors[:, :, vertex_pairs_list[i][1], :]], dim=-1)
...
class get_model_RRFSegNet(nn.Module):
    def __init__(self,name, points, is_training,  k=20, is_dist=True, weight_decay=0.0004,
                     bn_decay=None):
        ...
        relation_features1 = get_relation_features(points,nn_idx=nn_idx, k=k)
        self.net_1 = relation_reasoning_layers('layer_1', relation_features1,
        ...
        self.net_1.cuda()                                            
        ### layer_2
        relation_features2 = get_relation_features(self.net_1(relation_features1), nn_idx=nn_idx, k=k)
        self.net_2 = relation_reasoning_layers('layer_2', relation_features2,
                                            is_training=is_training, bn_decay=bn_decay,
        ...
        net1_out = self.net_1(relation_features1)
        net2_out = self.net_2(relation_features2)
        global_net_in = torch.cat([net1_out, net2_out], dim=3)
        global_net_in = global_net_in.permute(0,3,2,1)
        self.global_net = nn.Conv2d(global_net_in.shape[1], 1024, kernel_size=[1,1], stride=[1,1], padding=0)
        self.global_net.cuda()
        global_net,_ = torch.max(self.global_net(global_net_in), dim=-1, keepdim=True)
        global_net = global_net.repeat([1, 1, 1, num_point])
        global_net = global_net.permute(0,3,2,1)
        concat = torch.cat([global_net, net1_out, net2_out], dim=3)
